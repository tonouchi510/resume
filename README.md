...
コンピュータービジョン（CV）系の研究開発、設計・ソフトウェアアーキテクト、バックエンド開発（Golang, Python）を主な技術領域として取り組んでいます。
ただし、特に技術領域を限定しているわけではないので、（フロント・インフラ含め）必要に応じてキャッチアップし様々な技術を触るのが好きです。

以下経験詳細で記載の通り、長く運用している大規模サービスから、PoC的な技術検証含め、様々なステージの開発経験があります。
...
- 成果：
  ...
  - 自分の主な貢献：
    - 「サッカーのルールなどのドメインルールを組み込んだベイジアンネットワークの構築とMLモデルの事後確率補正」という新規手法を提案し、従来技術では困難だった課題を改善
      ...
      - => 論文化予定 & 特許出願済み・現在審査中
    - その他主メンバーとして、プロジェクトのマネジメントや全体的なモジュールの改善提案など
  - 外部露出
    - [会社のプレスリリース](https://mixi.co.jp/news/2025/0625/42339/)
    - 今後の露出予定：forbesの取材、解析パイプライン全体のレポート論文、新規提案技術の特許・論文周り
- 補足：
...
- 特徴抽出モデル更新における後方互換性という実務課題に取り組んだ話
  - 概要：別々に学習されたモデルの特徴空間は一般に異なるものになるため、特徴抽出モデルを更新すると古い特徴ベクトルと互換性がなくなり、比較ができない。そのまま新しいモデルに差し替える場合は過去に解析済みの特徴ベクトルをBackfill（再抽出）する必要がある。これを回避するために、当時のCVPRで提案されていたBackward Conpatible Trainingという、新しい特徴抽出モデルを学習する際に、旧特徴抽出モデルの特徴空間を共有するように（互換性があるように）、制約を加えた損失関数によって学習する手法を取り入れた
  ...
  - 補足：
    - ただし、実際には別の方法でモデル更新を行った（システム側でBackfillが不要になる対応を行った）
...
#### 画像・動画のタグ推定の導入:
- 工数を抑えてタグ推定を実現するために、当時最先端だった`OpenAI/CLIP`を利用したZero-shot（学習なし）のタグ推定を検証・導入
...
### その他
#### MLフレームワークの経験（例：SimCLRの分散学習を実装）
- 基本的には独自実装はTensorFlowが多い
...
#### 将棋の研究用アプリ
- 概要：
  - https://github.com/tonouchi510/shogi-lab
- 技術スタック：
...
### セキュリティ
- 大学時代にセキュリティの研究室に所属
  - ネットワークセキュリティ、暗号などを一定学習
- 卒業研究
  - SPT2018発表 "安全性と利便性を考慮したチャレンジ・レスポンス分離ユーザ認証に関する提案：Dummy Indicator"
- コンペ参加
  - MWS Cup 2017 2位（[リンク](https://www.iwsec.org/mws/2017/photo.html)）
  - マルウェア解析に関するコンペ
